...
export default function NavBar({ isCustomer, nameButtonOrder, linkOrder }) {
  ...
  return (
    <nav className="z-20 flex justify-between items-center text-xl fixed bg-white mb-15 max-w-full min-w-full font-medium shadow-md">
      ...
      { isCustomer
        && (
          <Link to="/customer/products">
            <div className="bg-yellow-color text-lg mx-10 px-10 py-1 rounded-md" data-testid="customer_products__element-navbar-link-products">
              Produtos
            </div>
          </Link>)}

      <Link to={ linkOrder }>
        <div className="bg-yellow-color text-lg ml-10 px-10 py-1 rounded-md" data-testid="customer_products__element-navbar-link-orders">
          {nameButtonOrder}
        </div>
      </Link>
      <div
      ...
      >
        <svg xmlns="http://www.w3.org/2000/svg" className="w-8 h-8 mx-2 text-lg text-yellow-color" viewBox="0 0 20 20" fill="currentColor">
      ...
      <Link to="/login" data-testid="customer_products__element-navbar-link-logout">
        <button
          ...
          className="rounded-md text-lg bg-dark-color text-yellow-color mx-10 px-5 py-1"
        >
          Sair
        </button>
      </Link>
